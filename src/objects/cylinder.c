...
/*   Updated: 2026/01/27 16:21:49 by wxi              ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	init_cylinder_transform(t_cylinder *cy)
{
	...
	s = mat_scaling(cy->diameter / 2, cy->height / 2, cy->diameter / 2);
...
double	intersect_unit_cylinder(t_ray *unit_ray, t_cylinder *cy)
{
	...
	a = unit_ray->direction.x * unit_ray->direction.x + unit_ray->direction.z
		* unit_ray->direction.z;
	if (a < EPSILON)
		return (-1.0);
	b = 2 * unit_ray->origin.x * unit_ray->direction.x + 2 * unit_ray->origin.z
		* unit_ray->direction.z;
	c = unit_ray->origin.x * unit_ray->origin.x + unit_ray->origin.z
		* unit_ray->origin.z - 1;
	disc = b * b - 4 * a * c;
	if (disc < 0)
		return (-1.0);
